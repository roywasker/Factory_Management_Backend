...
class JwtService (
    ...
    private fun generateToken(
        ...
        val expiryDate = Date(currentTime.time + expiry)
        return Jwts.builder()
            .subject(userId)
            .claim("type", type)
    ...
    fun getUerIdFromToken(token: String): String{
        val claims = parseAllClaimsFromToken(token) ?: throw ResponseStatusException(HttpStatusCode.valueOf(401),"Invalid token .")
        return claims.subject
    }

    private fun parseAllClaimsFromToken(token: String): Claims? {
        val rowToken = if (token.startsWith("Bearer ")) {
